...
(function () {
    ...
    if (angular) {
        ...
        // ZidalJS: changed prefix Adal into Zidal
        var ZidalModule = angular.module('ZidalAngular', []);

        // ZidalJS: changed prefix adal into zidal
        ZidalModule.provider('zidalAuthenticationService', function () {
            var _adal = null;
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                var processHash = function (hash, event) {
                   ...
                   && _adal._openedWindows[_adal._openedWindows.length - 1].opener._adalInstance) {
                        ...
                        if (requestInfo.stateMatch) {
                            if (requestInfo.requestType === _adal.REQUEST_TYPE.RENEW_TOKEN) {
                                ...
                                if ($window.parent === $window && !_adal._callBackMappedToRenewStates[requestInfo.stateResponse]) {
                                    if (token) {
                                        // zidal: changed adal prefix to zidal
                                        $rootScope.$broadcast('zidal:acquireTokenSuccess', token);
                                    }
                                    else if (error && errorDescription) {
                                        // zidal: changed adal prefix to zidal
                                        $rootScope.$broadcast('zidal:acquireTokenFailure', errorDescription, error);
                                    }
                                }
                                ...
                                if (_oauthData.userName) {
                                    ...
                                    // zidal: changed adal prefix to zidal
                                    $rootScope.$broadcast('zidal:loginSuccess', token);
                                } else {
                                    $rootScope.$broadcast('zidal:loginFailure', errorDescription, error);
                                }
                            ...
                            // zidal: changed adal prefix to zidal
                            $rootScope.$broadcast('zidal:stateMismatch', errorDescription, error);
                        }
                    } else {
                        ...
                        if (!_oauthData.isAuthenticated && _oauthData.userName && !_adal._renewActive) {
                            ...
                            // zidal: changed adal prefix to zidal
                            var self = $injector.get('zidalAuthenticationService');
                            self.acquireToken(_adal.config.loginResource).then(function (token) {
                                ...
                                // zidal: changed adal prefix to zidal
                                $rootScope.$broadcast('zidal:loginFailure', errorParts[0], errorParts[1]);
                            });
                        }
                    }
                ...
                var loginHandler = function (loginStartPage) {
                    ...
                    if (_adal.config && _adal.config.localLoginUrl) {
                        ...
                        // zidal: changed adal prefix to zidal
                        $rootScope.$broadcast('zidal:loginRedirect');
                        _adal.login();
                    }
                };
                ...
                // zidal: changed adal prefix to zidal
                $window.addEventListener('zidal:popUpHashChanged', function (e) {
                    processHash(e.detail);
                });

                // zidal: changed adal prefix to zidal
                $window.addEventListener('zidal:popUpClosed', function (e) {
                    ...
                    if (_adal._loginInProgress) {
                        ...
                        // zidal: changed adal prefix to zidal
                        $rootScope.$broadcast('zidal:acquireTokenFailure', errorParts[0], errorParts[1]);
                ...
                return {
                    ...
                    acquireToken: function (resource) {
                        ...
                        _adal.acquireToken(resource, function (errorDesc, tokenOut, error) {
                            ...
                            if (error) {
                                // zidal: changed adal prefix to zidal
                                $rootScope.$broadcast('zidal:acquireTokenFailure', errorDesc, error);
                                _adal.error('Error when acquiring token for resource: ' + resource, error);
                                deferred.reject(errorDesc + "|" + error);
                            } else {
                                // zidal: changed adal prefix to zidal
                                $rootScope.$broadcast('zidal:acquireTokenSuccess', tokenOut);
        ...
        // zidal: changed adal prefix to zidal
        AdalModule.factory('ProtectedResourceInterceptor', ['zidalAuthenticationService', '$q', '$rootScope', '$templateCache', function (authService, $q, $rootScope, $templateCache) {
                        ...
                        if (tokenStored) {
                            ...
                            if (authService.loginInProgress()) {
                                if (authService.config.popUp) {
                                    ...
                                    // zidal: changed adal prefix to zidal
                                    $rootScope.$on('zidal:loginSuccess', function (event, token) {
                                        if (token) {
                                    ...
                                    // zidal: changed adal prefix to zidal
                                    $rootScope.$on('zidal:loginFailure', function (event, error) {
                                        if (error) {
                                            config.data = error;
                ...
                responseError: function (rejection) {
                    ...
                    if (rejection) {
                        if (rejection.status === 401) {
                            ...
                            // zidal: changed adal prefix to zidal
                            $rootScope.$broadcast('zidal:notAuthorized', rejection, resource);
                        }
                        else {
                            // zidal: changed adal prefix to zidal
                            $rootScope.$broadcast('zidal:errorResponse', rejection);
                        }
