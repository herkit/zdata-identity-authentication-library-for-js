...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext = function (config) {
        ...
        this.CONSTANTS = {
            ...
            ERROR: 'error',
            STORAGE: {
                // zidal: changed prefix adal into zidal
                TOKEN_KEYS: 'zidal.token.keys',
                ACCESS_TOKEN_KEY: 'zidal.access.token.key',
                EXPIRATION_KEY: 'zidal.expiration.key',
                STATE_LOGIN: 'zidal.state.login',
                STATE_RENEW: 'zidal.state.renew',
                NONCE_IDTOKEN: 'zidal.nonce.idtoken',
                SESSION_STATE: 'zidal.session.state',
                USERNAME: 'zidal.username',
                IDTOKEN: 'zidal.idtoken',
                ERROR: 'zidal.error',
                ERROR_DESCRIPTION: 'zidal.error.description',
                LOGIN_REQUEST: 'zidal.login.request',
                LOGIN_ERROR: 'zidal.login.error',
                RENEW_STATUS: 'zidal.token.renew.status',
                ANGULAR_LOGIN_REQUEST: 'zidal.angular.login.request'
            },
            RESOURCE_DELIMETER: '|',
            CACHE_DELIMETER: '||',
        ...
        // public
        this.instance = 'https://id.zdata.no/';
        this.config = {};
        this.callback = null;
        this.popUp = false;
    ...
    AuthenticationContext.prototype._loginPopup = function (urlNavigate, resource, callback) {
        ...
        var pollTimer = window.setInterval(function () {
            if (!popupWindow || popupWindow.closed || popupWindow.closed === undefined) {
                ...
                if (that.isAngular) {
                    // zidal: changed prefix adal into zidal
                    that._broadcast('zidal:popUpClosed', errorDesc + that.CONSTANTS.RESOURCE_DELIMETER + error);
                }
            ...
            try {
                ...
                if (encodeURI(popUpWindowLocation.href).indexOf(encodeURI(registeredRedirectUri)) != -1) {
                    if (that.isAngular) {
                        // zidal: changed prefix adal into zidal
                        that._broadcast('zidal:popUpHashChanged', popUpWindowLocation.hash);
                    }
                    else {
  ...
  */
    ...
    AuthenticationContext.prototype.logOut = function () {
        ...
        if (this.config.logOutUri) {
            ...
            urlNavigate = this.instance + 'connect/endsession?' + logout;
        }
    ...
    */
    ...
    AuthenticationContext.prototype._getNavigateUrl = function (responseType, resource) {
        ...
        var urlNavigate = this.instance + 'connect/authorize' + this._serialize(responseType, this.config, resource) + this._addLibMetadata();
